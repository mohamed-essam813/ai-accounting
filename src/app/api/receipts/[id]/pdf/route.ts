...
export async function GET(
  ...
  if (error || !p) return NextResponse.json({ error: "Receipt not found" }, { status: 404 });
  if ((p as { payment_type?: string }).payment_type !== "receipt") {
    return NextResponse.json({ error: "Not a customer receipt" }, { status: 409 });
  }
