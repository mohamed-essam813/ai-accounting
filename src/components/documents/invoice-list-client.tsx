...
import {
  DOCUMENT_LIST_DATE_STORAGE_KEY,
  loadStoredDateRange,
  rangeForPreset,
  saveStoredDateRange,
  type DatePresetId,
} from "@/lib/documents/document-list-dates";
...
import type { InvoiceListRow } from "@/lib/data/document-lists/invoices-list";
...
function workflowToUi(row: InvoiceListRow): WorkflowUiStatus {
...
export function InvoiceListClient() {
  ...
  useEffect(() => {
    const stored = loadStoredDateRange("invoices");
  ...
  useEffect(() => {
    ...
    saveStoredDateRange("invoices", { startDate, endDate });
  }, [startDate, endDate]);
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/document-lists/invoices?${queryString}`);
  ...
  useEffect(() => {
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const res = await fetch(`/api/contacts/search?q=${encodeURIComponent(cpQuery)}&kind=customer`);
          if (!res.ok) return;
  ...
  const exportRows = async (): Promise<InvoiceListRow[]> => {
    if (!startDate || !endDate) return [];
    ...
    const res = await fetch(`/api/document-lists/invoices?${sp}`);
  ...
  const downloadCsv = async () => {
    ...
    a.download = `${meta?.companyName ?? "Company"}Invoices${startDate}-${endDate}_${stamp}.csv`;
    a.click();
  ...
  const downloadXlsx = async () => {
    ...
    XLSX.writeFile(
      wb,
      `${meta?.companyName ?? "Company"}Invoices${startDate}-${endDate}_${new Date().toISOString().slice(0, 10).replace(/-/g, "")}.xlsx`,
    );
  };
  ...
  const downloadPdf = async () => {
    ...
    doc.setFontSize(11);
    doc.text(`${meta?.companyName ?? "Company"} — Invoices (${startDate} to ${endDate})`, 14, 16);
    autoTable(doc, {
      startY: 22,
      head: [["#", "Date", "Customer", "Total", "Paid", "Outstanding", "Due", "Status"]],
      body: rows.map((r) => [
        r.invoiceNumber ?? "—",
    ...
    doc.save(
      `${meta?.companyName ?? "Company"}Invoices${startDate}-${endDate}_${new Date().toISOString().slice(0, 10).replace(/-/g, "")}.pdf`,
    );
  };
  ...
  return (
    <DocumentListShell
      title="Invoices"
      subtitle="Sales invoices and drafts. Filter, review, and export."
      primaryAction={
        <Button asChild>
          <Link href="/prompt">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Popover open={cpOpen} onOpenChange={setCpOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="h-9">
                  Customers{counterpartyIds.length ? ` (${counterpartyIds.length})` : ""}
                </Button>
              </PopoverTrigger>
            ...
            <div>
              ...
              <Input
                placeholder="Number, customer, amount…"
                value={search}
                onChange={(e) => {
          ...
          {data ? (
            <div className="flex flex-wrap gap-6 text-sm">
              <div>
                ...
                <span className="font-medium tabular-nums">{currency(data.summary.totalInvoiced, "AED")}</span>
              </div>
              <div>
      ...
      {noDataEver ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
          ...
          <Button asChild variant="link" className="mt-2">
            <Link href="/prompt">Create your first invoice →</Link>
          </Button>
        </div>
      ) : empty ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
                ...
                return (
                  <TableRow key={row.rowKey} className={cn(showStrikeVoid && "opacity-70")}>
                    <TableCell className="font-mono text-sm">
                      {row.source === "posted" ? (
                        <Link href={`/invoices/${row.id}`} className="text-primary hover:underline">
                          {row.invoiceNumber ?? "—"}
                        </Link>
                      ) : (
                    ...
                    <TableCell className="max-w-[160px] truncate text-sm">{row.customerName ?? "—"}</TableCell>
  ...
  if (st === "posted") {
    ...
    return (
      <div className="flex flex-wrap justify-end gap-1">
        ...
        {unpaid ? (
          <Button variant="ghost" size="sm" type="button" onClick={() => toast.message("Send", { description: "Email delivery coming soon." })}>
            Send
          </Button>
        ) : null}
