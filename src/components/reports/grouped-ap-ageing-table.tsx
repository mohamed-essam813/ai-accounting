/**
 * Grouped AP Ageing Table with Pivot-Table Features
 * Excel Elimination Doctrine: Pivot-Table-Level Reporting
 * 
 * Features:
 * - Grouping by vendor
 * - Expand/collapse by vendor
...
import type { APAgeingItem, APAgeingSummary } from "@/lib/data/ageing";

interface Props {
  items: APAgeingItem[];
  summary?: APAgeingSummary[];
  showSummary?: boolean;
  displayCurrency?: string;
}

type SortField = "vendor" | "outstanding" | "days_overdue";
type SortDirection = "asc" | "desc";

export function GroupedAPAgeingTable({ items, summary, showSummary = true, displayCurrency = "AED" }: Props) {
  ...
  const grouped = items.reduce((acc, item) => {
    ...
    acc[vendor].bills.push(item);
    acc[vendor].totalCurrent += item.current_0_30;
    acc[vendor].total31_60 += item.days_31_60;
    acc[vendor].total61_90 += item.days_61_90;
    acc[vendor].total90Plus += item.days_90_plus;
    acc[vendor].totalOutstanding += item.outstanding_amount;
    acc[vendor].maxDaysOverdue = Math.max(acc[vendor].maxDaysOverdue, item.days_overdue);
    return acc;
  }, {} as Record<string, {
    vendor: string;
    bills: APAgeingItem[];
  ...
  return (
    <div className="space-y-4">
      {/* Summary Table (Vendor-level totals) */}
                  ...
                  return (
                    <Fragment key={group.vendor}>
                      ...
                      <TableRow className="bg-muted/50 font-semibold hover:bg-muted/70">
                        ...
                        <TableCell className="font-medium">{group.vendor}</TableCell>
                        <TableCell colSpan={3}>
                          {hasOverdue && (
                      ...
                      {/* Vendor Bills (when expanded) */}
                      {isExpanded &&
                        group.bills
                          .sort((a, b) => b.days_overdue - a.days_overdue)
                          .map((item) => (
                            <TableRow key={`${item.vendor_name}-${item.bill_number}`} className="hover:bg-muted/30">
                              <TableCell></TableCell>
                              <TableCell className="pl-6">{item.vendor_name}</TableCell>
                              <TableCell className="font-mono text-xs">{item.bill_number || "—"}</TableCell>
