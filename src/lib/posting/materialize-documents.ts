...
export async function materializeInvoiceOrBillFromPostedDraft(
  ...
  const taxAmount = taxFromEntities ?? taxFromDraft ?? 0;
  const subtotal = Math.max(amount - taxAmount, 0);
  const currencyCode =
    (entities.currency as string) ||
    (draftData.currency as string | undefined) ||
    null;
  const dueRaw = entities.due_date as string | null | undefined;
  const invoiceNumber = (entities.invoice_number as string | null | undefined) ?? null;
