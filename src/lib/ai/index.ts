...
export async function parseAccountingPrompt(prompt: string, context: ParsePromptContext) {
  ...
  const systemPrompt = [
    ...
    "4. Generate ALL required journal lines (journal_lines array). Each line MUST include account_code, debit, credit, and a role field:",
    "   Sales invoices: receivable | revenue | output_vat | cogs | inventory_relief | discount | rounding",
    "   Supplier bills: payable | expense | inventory_receipt | fixed_asset | input_vat",
    "   Use cogs + inventory_relief only for perpetual-inventory product sales; never invent COGS amounts — engine computes them at post.",
    "   Do not rely on account_name for posting; names come from the chart of accounts.",
